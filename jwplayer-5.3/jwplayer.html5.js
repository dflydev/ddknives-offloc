	...
	jwplayer.html5.view = function(api, container, model) {
		...
		function createWrapper() {
			...
			_css(_wrapper, {
				...
				margin: 'auto',
				padding: 0,
				background: api.skin.getComponentSettings("display").backgroundcolor === undefined ? "" : api.skin.getComponentSettings("display").backgroundcolor.replace("0x", "#"),
			...
			if (model.getMedia() !== undefined && !model.getMedia().hasChrome && !model.config.chromeless) {
				for (var pluginIndex in model.plugins.order) {
					...
					if (model.plugins.object[pluginName].getDisplayElement !== undefined) {
						...
						model.plugins.style[pluginName] = plugincss;
					}
				}
			} else {
		...
		this.setup = function(container) {
			...
			window.onresize = function(evt) {
				if (api.jwGetFullscreen()) {
					model.width = document.body.clientWidth;
					model.height = document.body.clientHeight;
				}
				_resize(model.width, model.height);
			};
		};
		...
		function _resize(width, height) {
			...
			for (var pluginIndex in model.plugins.order) {
				var pluginName = model.plugins.order[pluginIndex];
				var style = model.plugins.style[pluginName];
				if (api.jwGetFullscreen()){
					style = jwplayer.utils.extend({}, model.plugins.style[pluginName], {
						position: "fixed",
						top: 0,
						left: 0
					});	
				}
				_css(model.plugins.object[pluginName].getDisplayElement(), style);
				if (model.plugins.config[pluginName].position == jwplayer.html5.view.positions.OVER && !api.jwGetFullscreen()) {
					...
					model.plugins.object[pluginName].resize(model.width, model.height);
				}
			}
			if (!model.fullscreen) {
				_css(model.getMedia().getDisplayElement(), {
					position: 'absolute',
					width: (model.width - _box.left - _box.right),
					height: (model.height - _box.top - _box.bottom),
					top: _box.top,
					left: _box.left
				});
			}
		...
		this.fullscreen = function(state) {
			if (model.getMedia().getDisplayElement().webkitSupportsFullscreen) {
				if (state) {
					model.height = screen.availHeight;
					model.width = screen.availWidth;
					model.getMedia().getDisplayElement().webkitEnterFullscreen();
				} else {
					model.height = _height;
					model.width = _width;
					model.getMedia().getDisplayElement().webkitExitFullscreen();
				}
			} else {
				if (state) {
					model.width = document.body.clientWidth;
...
(function(jwplayer) {
	...
	jwplayer.html5.controlbar = function(api, config) {
		...
		this.resize = function(width, height) {
			if (_wrapper.parentElement !== undefined && !_ready) {
				_ready = true;
				document.getElementById(api.id).onmousemove = _fadeOut;
			}
			_width = width;
			_height = height;
			if (api.jwGetFullscreen()) {
		...
		function _fadeOut() {
			...
			_fadeOutTimeout = setTimeout(function() {
				...
				jwplayer.html5.utils.fadeTo(_wrapper, 0, 1, 1);
		...
		function _resizeBar() {
			...
			var elementcss = {};
			if (_settings.position.toUpperCase() == jwplayer.html5.view.positions.OVER || api.jwGetFullscreen()) {
		...
		function _stateHandler(obj) {
			...
			switch (api.jwGetState()) {
				...
				case jwplayer.api.events.state.IDLE:
					...
					var background = api.jwGetPlaylist()[api.jwGetItem()].image === "" ? "" : " url('" + jwplayer.html5.utils.getAbsolutePath(api.jwGetPlaylist()[api.jwGetItem()].image) + "')";
					_css(_display.display_image, {
